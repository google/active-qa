...
def create_all(args):
  ...
  train_data = json.load(tf.gfile.Open(train_path, 'rb'))
  dev_path = os.path.join(args.source_dir, 'dev-v1.1.json')
  dev_data = json.load(tf.gfile.Open(dev_path, 'rb'))
  train_data['data'].extend(dev_data['data'])
  print('dumping all data ...')
  json.dump(train_data, tf.gfile.Open(out_path, 'wb'))


def prepro(args):
  if not tf.gfile.Exists(args.target_dir):
    tf.gfile.MakeDirs(args.target_dir)
...
def save(args, data, shared, data_type):
  ...
  shared_path = os.path.join(args.target_dir,
                             'shared_{}.json'.format(data_type))
  json.dump(data, tf.gfile.Open(data_path, 'wb'))
  json.dump(shared, tf.gfile.Open(shared_path, 'wb'))


def get_word2vec(args, word_counter):
  glove_path = os.path.join(args.glove_dir, 'glove.{}.{}d.txt'.format(
      args.glove_corpus, args.glove_vec_size))
  sizes = {
      '6B': int(4e5),
      '42B': int(1.9e6),
      '840B': int(2.2e6),
  ...
  word2vec_dict = {}
  with tf.gfile.Open(glove_path, 'r') as fh:
