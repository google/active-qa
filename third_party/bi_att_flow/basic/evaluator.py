...
from third_party.bi_att_flow.my.nltk_utils import span_f1
from third_party.bi_att_flow.my.tensorflow.general import padded_reshape
from third_party.bi_att_flow.my.utils import argmax
from third_party.bi_att_flow.squad.utils import get_phrase, get_best_span
...
class LabeledEvaluation(Evaluation):
  ...
  def __init__(self, data_type, global_step, idxs, yp, y, tensor_dict=None):
    super(LabeledEvaluation, self).__init__(
        data_type, global_step, idxs, yp, tensor_dict=tensor_dict)
    self.y = y
    self.dict['y'] = y
...
class AccuracyEvaluation(LabeledEvaluation):
  ...
  def __init__(self,
    ...
    super(AccuracyEvaluation, self).__init__(
        data_type, global_step, idxs, yp, y, tensor_dict=tensor_dict)
    self.loss = loss
    ...
    loss_summary = tf.Summary(value=[
        tf.Summary.Value(
            tag='{}/loss'.format(data_type), simple_value=self.loss)
    ])
    acc_summary = tf.Summary(value=[
        tf.Summary.Value(tag='{}/acc'.format(data_type), simple_value=self.acc)
    ])
    self.summaries = [loss_summary, acc_summary]

  def __repr__(self):
    return '{} step {}: accuracy={}, loss={}'.format(
        self.data_type, self.global_step, self.acc, self.loss)
  ...
  def __add__(self, other):
    ...
    new_loss = (self.loss * self.num_examples + other.loss * other.num_examples
               ) / len(new_correct)
    if self.tensor_dict is not None:
      new_tensor_dict = {
          key: np.concatenate((val, other.tensor_dict[key]), axis=0)
...
class LabeledEvaluator(Evaluator):
  ...
  def __init__(self, config, model, tensor_dict=None):
    super(LabeledEvaluator, self).__init__(
        config, model, tensor_dict=tensor_dict)
    self.y = model.y
...
class AccuracyEvaluator(LabeledEvaluator):
  ...
  def get_evaluation(self, sess, batch):
    ...
    global_step, yp, loss, vals = sess.run(
        [self.global_step, self.yp, self.loss, list(self.tensor_dict.values())],
        feed_dict=feed_dict)
    y = data_set.data['y']
    yp = yp[:data_set.num_examples]
...
class ForwardEvaluation(Evaluation):
  ...
  def __init__(self,
               ...
               id2answer_dict,
               tensor_dict=None):
    super(ForwardEvaluation, self).__init__(
        data_type, global_step, idxs, yp, tensor_dict=tensor_dict)
    self.yp2 = yp2
    self.loss = loss
    self.dict['loss'] = loss
  ...
  def __add__(self, other):
    ...
    new_id2answer_dict = dict(
        list(self.id2answer_dict.items()) + list(other.id2answer_dict.items()))
    new_id2score_dict = dict(
        list(self.id2answer_dict['scores'].items()) + list(
            other.id2answer_dict['scores'].items()))
  ...
  def __repr__(self):
    return '{} step {}: loss={:.4f}'.format(self.data_type, self.global_step,
                                            self.loss)
...
class F1Evaluation(AccuracyEvaluation):
  ...
  def __init__(self,
    ...
    f1_summary = tf.Summary(value=[
        tf.Summary.Value(tag='{}/f1'.format(data_type), simple_value=self.f1)
    ])
  ...
  def __add__(self, other):
    ...
    new_id2answer_dict['scores'] = new_id2score_dict
    new_id2f1_dict = dict(
        list(self.id2answer_dict['f1_scores'].items()) +
        list(other.id2answer_dict['f1_scores'].items()))
    new_id2answer_dict['f1_scores'] = new_id2f1_dict
    return F1Evaluation(self.data_type, self.global_step, new_idxs, new_yp,
                        new_yp2, new_y, new_correct, new_loss, new_f1s,
                        new_id2answer_dict)

  def __repr__(self):
    return '{} step {}: accuracy={:.4f}, f1={:.4f}, loss={:.4f}'.format(
        self.data_type, self.global_step, self.acc, self.f1, self.loss)
...
class F1Evaluator(LabeledEvaluator):
  ...
  def get_evaluation(self, sess, batch):
    ...
    id2answer_dict = {
        ...
        for id_, xi, span, context in zip(data_set.data['ids'], data_set.data[
            'x'], spans, data_set.data['p'])
    }
    id2score_dict = {
        id_: score
...
class MultiGPUF1Evaluator(F1Evaluator):
  ...
  def __init__(self, config, models, tensor_dict=None):
    super(MultiGPUF1Evaluator, self).__init__(
        config, models[0], tensor_dict=tensor_dict)
    self.models = models
...
class ForwardEvaluator(Evaluator):
  ...
  def __init__(self, config, model, tensor_dict=None):
    super(ForwardEvaluator, self).__init__(
        config, model, tensor_dict=tensor_dict)
    self.yp2 = model.yp2
    self.loss = model.loss
