...
class DataSet(object):
  ...
  def get_multi_batches(self,
    ...
    multi_batches = (tuple(
        zip(
            grouper(
                idxs, batch_size, shorten=True, num_groups=
                num_batches_per_step), data_set.divide(num_batches_per_step)))
  ...
  def __add__(self, other):
    ...
    valid_idxs = list(self.valid_idxs) + [
        valid_idx + self.num_examples for valid_idx in other.valid_idxs
    ]
    return DataSet(
        data, self.data_type, shared=self.shared, valid_idxs=valid_idxs)

  def divide(self, integer):
    batch_size = int(math.ceil(self.num_examples / integer))
    idxs_gen = grouper(
        self.valid_idxs, batch_size, shorten=True, num_groups=integer)
    data_gen = (self.get_by_idxs(idxs) for idxs in idxs_gen)
    ds_tuple = tuple(
        DataSet(data, self.data_type, shared=self.shared) for data in data_gen)
    return ds_tuple


def load_metadata(config, data_type):
  metadata_path = os.path.join(config.data_dir,
...
def read_data(config, data_type, ref, data_filter=None):
  ...
  shared_path = os.path.join(config.data_dir,
                             'shared_{}.json'.format(data_type))
  ...
  print('Loaded {}/{} examples from {}'.format(
      len(valid_idxs), num_examples, data_type))

  shared_path = config.shared_path or os.path.join(config.out_dir,
                                                   'shared.json')

  # Drop cased word embeddings if these are not used
  if config.lower_word:
    shared.pop('word2vec')

  # Characters are taken from shared['x']
  if 'cx' in shared:
    shared.pop('cx')

  if not ref:
    word2vec_dict = shared['lower_word2vec'] if config.lower_word else shared[
        'word2vec']
    word_counter = shared[
        'lower_word_counter'] if config.lower_word else shared['word_counter']
