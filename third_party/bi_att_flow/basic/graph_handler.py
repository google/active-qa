...
from third_party.bi_att_flow.basic.evaluator import Evaluation, F1Evaluation
from third_party.bi_att_flow.my.utils import short_floats
...
class GraphHandler(object):
  ...
  def initialize(self, sess):
    ...
    if self.config.mode == "train":
      self.writer = tf.summary.FileWriter(
          self.config.log_dir, graph=tf.get_default_graph())
  ...
  def _load(self, sess):
    ...
    if config.load_path:
      ...
      save_path = os.path.join(config.save_dir, "{}-{}".format(
          config.model_name, config.load_step))
    else:
      save_dir = config.save_dir
      checkpoint = tf.train.get_checkpoint_state(save_dir)
      assert checkpoint is not None, "cannot load checkpoint at {}".format(
          save_dir)
  ...
  def dump_eval(self, e, precision=2, path=None):
    ...
    if self.config.dump_pickle:
      path = path or os.path.join(self.config.eval_dir, "{}-{}.pklz".format(
          e.data_type, str(e.global_step).zfill(6)))
      with gzip.GzipFile(
          fileobj=tf.gfile.FastGFile(path, "wb"), compresslevel=3) as fh:
        pickle.dump(e.dict, fh)
    else:
      path = path or os.path.join(self.config.eval_dir, "{}-{}.json".format(
          e.data_type, str(e.global_step).zfill(6)))
      with tf.gfile.Open(path, "w") as fh:
        json.dump(short_floats(e.dict, precision), fh)

  def dump_answer(self, e, path=None):
    assert isinstance(e, Evaluation)
    path = path or os.path.join(self.config.answer_dir, "{}-{}.json".format(
        e.data_type, str(e.global_step).zfill(6)))
    with tf.gfile.Open(path, "w") as fh:
