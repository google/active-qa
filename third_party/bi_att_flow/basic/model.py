...
from third_party.bi_att_flow.my.tensorflow.nn import softsel, get_logits, highway_network, multi_conv1d
from third_party.bi_att_flow.my.tensorflow.rnn import bidirectional_dynamic_rnn
from third_party.bi_att_flow.my.tensorflow.rnn_cell import SwitchableDropoutWrapper, AttentionCell


def get_model(config):
  """ Returns a single model running on one CPU / GPU"""
  with tf.device('/{}:0'.format(config.device_type)):
    with tf.name_scope('model_0') as scope:
      model = Model(config, scope, rep=True)
  return [model]
...
class Model(object):
  ...
  def __init__(self, config, scope, rep=True):
    ...
    self.new_emb_mat = tf.placeholder(
        'float', [None, config.word_emb_size], name='new_emb_mat')
  ...
  def _build_forward(self):
    ...
    # Setting the scope in order to limit variable reusability.
    with tf.variable_scope('emb'):
      if config.use_char_emb:
        with tf.variable_scope('emb_var'), tf.device('/cpu:0'):
          char_emb_mat = tf.get_variable(
              'char_emb_mat', shape=[VC, dc], dtype='float')
      ...
      if config.use_word_emb:
        with tf.variable_scope('emb_var'), tf.device('/cpu:0'):
          if config.mode == 'train':
            ...
            word_emb_mat = tf.get_variable(
                'word_emb_mat', shape=[VW, dw], dtype='float')
    ...
    with tf.variable_scope('prepro'):
      d_fw_cell = SwitchableDropoutWrapper(
          tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
          self.is_train,
          input_keep_prob=config.input_keep_prob)
      d_bw_cell = SwitchableDropoutWrapper(
          tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
          self.is_train,
          input_keep_prob=config.input_keep_prob)
      (fw_u, bw_u), ((_, fw_u_f), (_, bw_u_f)) = bidirectional_dynamic_rnn(
          d_fw_cell, d_bw_cell, qq, q_len, dtype='float',
          scope='u1')  # [N, J, d], [N, d]
      u = tf.concat([fw_u, bw_u], 2)
      if config.share_lstm_weights:
        tf.get_variable_scope().reuse_variables()
        fw_cell = tf.contrib.rnn.BasicLSTMCell(
            d, state_is_tuple=True, reuse=True)
        bw_cell = tf.contrib.rnn.BasicLSTMCell(
            d, state_is_tuple=True, reuse=True)
        (fw_h, bw_h), _ = bidirectional_dynamic_rnn(
            fw_cell, bw_cell, xx, x_len, dtype='float',
            scope='u1')  # [N, M, JX, 2d]
        h = tf.concat([fw_h, bw_h], 3)  # [N, M, JX, 2d]
      else:
        fw_cell = tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True)
        bw_cell = tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True)
        (fw_h, bw_h), _ = bidirectional_dynamic_rnn(
            fw_cell, bw_cell, xx, x_len, dtype='float',
            scope='h1')  # [N, M, JX, 2d]
        h = tf.concat([fw_h, bw_h], 3)  # [N, M, JX, 2d]
    ...
    with tf.variable_scope('main'):
      if config.dynamic_att:
        p0 = h
        u = tf.reshape(
            tf.tile(tf.expand_dims(u, 1), [1, M, 1, 1]), [N * M, JQ, 2 * d])
        q_mask = tf.reshape(
            tf.tile(tf.expand_dims(self.q_mask, 1), [1, M, 1]), [N * M, JQ])
        first_fw_cell_0 = AttentionCell(
            tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
            u,
            mask=q_mask,
            mapper='sim',
        ...
        first_fw_cell_0 = SwitchableDropoutWrapper(
            tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
            self.is_train,
            input_keep_prob=config.input_keep_prob)
        first_bw_cell_0 = SwitchableDropoutWrapper(
            tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
            self.is_train,
            input_keep_prob=config.input_keep_prob)

        first_fw_cell_1 = SwitchableDropoutWrapper(
            tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
            self.is_train,
            input_keep_prob=config.input_keep_prob)
        first_bw_cell_1 = SwitchableDropoutWrapper(
            tf.contrib.rnn.BasicLSTMCell(d, state_is_tuple=True),
            self.is_train,
            input_keep_prob=config.input_keep_prob)
      ...
      logits = get_logits(
          ...
          scope='logits1')
      a1i = softsel(
          tf.reshape(g1, [N, M * JX, 2 * d]), tf.reshape(logits, [N, M * JX]))
  ...
  def _build_loss(self):
    ...
    self.loss = tf.add_n(
        tf.get_collection('losses', scope=self.scope), name='loss')
  ...
  def _build_ema(self):
    ...
    ema = self.ema
    tensors = tf.get_collection(
        'ema/scalar', scope=self.scope) + tf.get_collection(
            'ema/vector', scope=self.scope)
  ...
  def get_feed_dict(self, batch, is_train, supervised=True):
    ...
    if config.len_opt:
      """
            Note that this optimization results in variable GPU RAM usage (i.e.
            can cause OOM in the middle of training.)
    ...
    if supervised:
      ...
      assert not config.single

      X = batch.data['x']
      for i, (xi, yi) in enumerate(zip(X, batch.data['y'])):
        if len(yi) == 0:
          continue
    ...
    for i, xi in enumerate(X):
      ...
      for j, xij in enumerate(xi):
        ...
        for k, xijk in enumerate(xij):
          ...
          for l, xijkl in enumerate(xijk):
            if l == config.max_word_size:
              break
            cx[i, j, k, l] = _get_char(xijkl)  # batch, sentence, word, char
...
def bi_attention(config,
  ...
  with tf.variable_scope(scope or 'bi_attention'):
    ...
    if h_mask is None:
      ...
      u_mask_aug = tf.tile(
          tf.expand_dims(tf.expand_dims(u_mask, 1), 1), [1, M, JX, 1])
...
def attention_layer(config,
  ...
  with tf.variable_scope(scope or 'attention_layer'):
    ...
    if not config.c2q_att:
      u_a = tf.tile(
          tf.expand_dims(tf.expand_dims(tf.reduce_mean(u, 1), 1), 1),
          [1, M, JX, 1])
    if config.q2c_att:
