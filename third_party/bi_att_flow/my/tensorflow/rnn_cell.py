...
class SwitchableDropoutWrapper(tf.nn.rnn_cell.DropoutWrapper):
...
class NoOpCell(tf.nn.rnn_cell.RNNCell):
...
class MatchCell(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with tf.variable_scope(scope or self.__class__.__name__):
      ...
      f = tf.tanh(
          linear(
              [qs, x_tiled, h_prev_tiled], self._input_size, True,
              scope='f'))  # [N, JQ, d]
      a = tf.nn.softmax(
...
class AttentionCell(tf.nn.rnn_cell.RNNCell):
  ...
  def __call__(self, inputs, state, scope=None):
    with tf.variable_scope(scope or 'AttentionCell'):
      ...
      sel_mem = softsel(
          self._flat_memory, memory_logits, mask=self._flat_mask)  # [N, m]
  ...
  def get_double_linear_controller(size,
    ...
    def double_linear_controller(inputs, state, memory):
      ...
      out = double_linear_logits(
          in_, size, bias, input_keep_prob=input_keep_prob, is_train=is_train)
      return out
  ...
  def get_sim_mapper():
    ...
    def sim_mapper(inputs, state, sel_mem):
      """
            ...
            """
      return tf.concat(
          1, [inputs, sel_mem, inputs * sel_mem,
              tf.abs(inputs - sel_mem)]), state
