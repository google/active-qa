...
class IteratorUtilsTest(tf.test.TestCase):
  ...
  def testGetIteratorWithMaxLength(self):
    tf.set_random_seed(1)
    tgt_vocab_table = src_vocab_table = lookup_ops.index_table_from_tensor(
        tf.constant(["a", "b", "c", "eos", "sos"]))
    src_dataset = tf.data.Dataset.from_tensor_slices(
        tf.constant(["f e a g", "c c a", "d", "c a"]))
    tgt_dataset = tf.data.Dataset.from_tensor_slices(
        tf.constant(["c c c c", "a b c c", "", "b c"]))
    hparams = tf.contrib.training.HParams(
        random_seed=3,
        num_buckets=5,
        eos="eos",
    ...
    with self.test_session() as sess:
      ...
      self.assertAllEqual(
          ...
          target_output_v)
      self.assertAllEqual([4, 4], tgt_len_v)
      ...
      self.assertAllEqual(
          [[2, 0]],  # c a
          source_v)
      self.assertAllEqual([2], src_len_v)
      self.assertAllEqual(
          [[4, 1, 2]],  # sos b c
          target_input_v)
      self.assertAllEqual(
  ...
  def testGetInferIterator(self):
    ...
    hparams = tf.contrib.training.HParams(
        random_seed=3, eos="eos", sos="sos", context_feed="")
    batch_size = 2
    src_max_len = 3
