...
__all__ = [
    ...
    "run_full_eval", "init_stats", "update_stats", "print_step_info",
    "process_stats", "train"
]


def run_sample_decode(infer_model, infer_sess, hparams, summary_writer,
                      src_data, ctx_data, tgt_data, annot_data):
  """Sample decode a random sentence from src_data."""
  with infer_model.graph.as_default():
    global_step = model_helper.get_global_step(infer_model.model, infer_sess)

  _sample_decode(infer_model.model, global_step, infer_sess, hparams,
...
def run_internal_eval(eval_model,
  ...
  if use_test_set and hparams.test_prefix:
    ...
    if test_ctx_file is not None:
      test_iterator_feed_dict[eval_model.ctx_file_placeholder] = test_ctx_file
...
def run_external_eval(infer_model,
  ...
  with infer_model.graph.as_default():
    global_step = model_helper.get_global_step(infer_model.model, sess)
  ...
  if dev_ctx_file is not None:
    dev_iterator_feed_dict[infer_model.ctx_placeholder] = inference.load_data(
        dev_ctx_file)

  if hparams.dev_annotations is not None:
    dev_iterator_feed_dict[infer_model.annot_placeholder] = inference.load_data(
  ...
  if use_test_set and hparams.test_prefix:
    ...
    if hparams.test_annotations is not None:
      test_iterator_feed_dict[
          infer_model.annot_file_placeholder] = inference.load_data(
...
def run_full_eval(infer_model, infer_sess, eval_model, eval_sess, hparams,
  ...
  run_sample_decode(infer_model, infer_sess, hparams, summary_writer,
  ...
  if not hparams.use_rl:
    dev_ppl, test_ppl = run_internal_eval(eval_model, eval_sess, hparams,
                                          summary_writer)

  dev_scores, test_scores, global_step = run_external_eval(
      infer_model, infer_sess, hparams, summary_writer)
...
def update_stats(stats, start_time, step_result):
  ...
  (_, step_loss, step_reward, step_predict_count, step_summary, global_step,
...
def train(hparams, scope=None, target_session=""):
  ...
  train_model = model_helper.create_train_model(
      model_creator, hparams, scope, graph=combined_graph)
  ...
  sess = tf.Session(
      target=target_session, config=config_proto, graph=combined_graph)
  ...
  stats, info, start_train_time = before_train(
      loaded_train_model, train_model, sess, global_step, hparams, log_f)
  while global_step < num_train_steps:
    ...
    if global_step - last_eval_step >= steps_per_eval:
      ...
      if not hparams.use_rl:
        dev_ppl, test_ppl = run_internal_eval(eval_model, sess, hparams,
                                              summary_writer)
  ...
  (result_summary, _, final_eval_metrics) = (
      run_full_eval(infer_model, sess, eval_model, sess, hparams,
                    summary_writer, sample_src_data, sample_ctx_data,
                    sample_tgt_data, sample_annot_data))
