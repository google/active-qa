...
__all__ = [
    "load_data", "inference", "single_worker_inference",
...
def inference(ckpt,
  ...
  inference_context_file = None
  if hparams.ctx is not None:
    inference_context_file = "%s.%s" % (inference_input_file, hparams.ctx)
...
def single_worker_inference(infer_model, ckpt, inference_input_file,
  ...
  with tf.Session(
    ...
    loaded_infer_model = model_helper.load_model(infer_model.model, ckpt, sess,
                                                 "infer")
...
def multi_worker_inference(infer_model, ckpt, inference_input_file,
  ...
  with tf.Session(
    ...
    with codecs.getwriter("utf-8")(tf.gfile.GFile(
        final_output_infer, mode="wb")) as final_f:
